...
async def upload_pfp(file: ReqUploadFile, uid: LoggedInUID, db: DBSession):
    ...
    q_user = select(User).where(User.user_id == uid).limit(1)
    ...
    async with httpx.AsyncClient() as cli:
        res = await cli.post(
            ...
            files={"file": (req_filename, file_bytes, file.content_type)},
        )
...
async def fetch_pfp_from_username(username: str, db: DBSession):
    ...
    return {"url": media_url(MediaType.USER_PROFILE, f"{username}_{s_suffix}")}
